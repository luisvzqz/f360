...
class F360Amortiza(models.Model):
    ...
    x_company_id = fields.Many2one(string="Company id", store=True, copy=True, help="Company id", comodel_name="res.company", on_delete=NULL)
    ...
    x_opportunity_id = fields.Many2one(string="Opportunity id", store=True, copy=True, comodel_name="crm.lead", on_delete=NULL, help="Opportunity id")
    x_order_id = fields.Many2one(string="Order id", required=False, comodel_name="sale.order", on_delete=NULL, help="Order id")
    x_partner_id = fields.Many2one(string="Código partner id", store=True, copy=True, help="Partner id", comodel_name="res.partner", on_delete=NULL)
    x_pay = fields.Float(string="Payment", compute='_compute_x_pay', store=True, copy=True, help="Payment")
    x_pay_tax = fields.Float(string="Pay with Tax", compute='_compute_x_pay_tax', store=True, copy=True, help="pay with tax")
    x_product_id = fields.Many2one(string="Product id", store=True, copy=True, help="Product id", comodel_name="product.product", on_delete=NULL)
